 ...
 *
 * ## Connection modes
 * Supports two modes — configure [port2SysfsPrefix] and [port3SysfsPrefix] accordingly:
 *
 * ### Mode A – Via USB-C Hub (single physical port)
 * Insert a USB-C Hub into one physical port, then plug one TypeC USB 2.0 and one TypeC USB 3.0 stick into it.
 * ```
 ...
 *
 * ### Mode B – Direct (two separate USB-C ports or one port with two USB-C types)
 * Plug TypeC USB 2.0 stick directly into one port and TypeC USB 3.0 into another.
 * ```
 ...
 *
 * ## Test flow
 * Each port runs its own independent coroutine: **detect → (auto-format if needed) → speed test**.
 * Whichever stick is inserted first starts testing first. Both must pass for overall success.
 *
 ...
 * @property detectTimeoutSecs Hard timeout (seconds) for the entire test; triggers FAIL when exceeded.
 ...
 * @property port3MinWriteSpeedMb TypeC USB 3.0 minimum write speed (MB/s). Branded sticks: ≥10 MB/s.
 ...
 * - WAITING_REPLUG   is entered after the cable is unplugged; OP must re-plug in opposite orientation.
...
data class UsbCFunctionalTestUiState(
  ...
  val overallInstruction: String = "Please insert TypeC USB 2.0 and TypeC USB 3.0 devices and HDMI display as well.",
 ...
 *
 * ## Device detection
 * The `/sys/block/sdX` symlink encodes the USB topology. Both modes are supported:
 * ```
 ...
 *
 * ## Test flow
 * Each port runs **independently** in its own coroutine:
 * ```
 ...
 *
 * ## Auto-format
 * If a non-FAT32 stick is detected and [UsbCFunctionalTestArgs.autoFormatToFat32] is `true`,
 * the device is formatted once per run via Android's `sm partition` command (vold).
 */
class UsbCFunctionalTestViewModel : FactoryActionViewModel<UsbCFunctionalTestArgs>() {
  ...
  /** Block devices already formatted this run (prevents infinite retry). Thread-safe for concurrent access. */
  ...
  override suspend fun runActionImpl(): FactoryActionResult {
    ...
    return try {
      withTimeout(args.detectTimeoutSecs * 1000L) {
        coroutineScope {
          val firstCyclePassed = runUsbDisplayCycle(
          ...
          if (args.requireReverseCycle) {
            _ui.update {
              it.copy(overallInstruction = "Front cycle passed. Flip the TypeC cable to start the reverse cycle.")
            }
            prepareForNextCycle()

            val secondCyclePassed = runUsbDisplayCycle(
      ...
      _ui.update { it.copy(overallInstruction = "Timeout: test did not finish within ${args.detectTimeoutSecs}s.") }
      ...
      it.copy(
        ...
        hdmiPort =
          if (args.enableDisplayTest) {
            ...
            UsbCPortState(label = "TypeC HDMI Display", isFinished = true, isPassed = true, status = "SKIPPED")
          },
        hdmiTestPhase = null,
  ...
  private suspend fun runUsbDisplayCycle(
    ...
    coroutineScope {
      if (expectedCc != null) {
        waitForExpectedCc(expectedCc, cycleLabel)
      }
      _ui.update { it.copy(overallInstruction = "Running $cycleLabel. Keep the device connected and wait for the test to finish.") }
      ...
      val port1Job = async(factoryContext.ioDispatcher) {
        runPortTest(
      ...
      val port2Job = async(factoryContext.ioDispatcher) {
        runPortTest(
      ...
      val hdmiJob = if (args.enableDisplayTest) {
        async(factoryContext.ioDispatcher) { runHdmiTest() }
      } else {
        null
      }
      val ccPollJob = if (ccEnabled) {
  ...
  private suspend fun waitForExpectedCc(expectedCc: String, cycleLabel: String) {
    while (true) {
      ...
      when (currentCc) {
        ...
        "CC1", "CC2" -> {
          _ui.update {
            it.copy(
              ccPhase = TypeCCcPhase.DETECTING_FIRST(detected = currentCc),
              overallInstruction =
                if (expectedCc == "CC1") {
                  "The first insertion must be CC1. Flip the USB and try again."
                } else {
                  "The second insertion must be CC2. Flip the USB and try again."
                },
            )
          }
          delay(500)
          ...
          _ui.update {
            it.copy(
              ccPhase = TypeCCcPhase.DETECTING_FIRST(),
              overallInstruction =
                if (expectedCc == "CC1") {
                  "Insert the TypeC cable in the front direction first. Only CC1 is accepted."
                } else {
                  "Flip the TypeC cable and reinsert it. Only CC2 is accepted for the second cycle."
                },
            )
          }
          delay(500)
  ...
  private suspend fun readCcOrientation(): String? =
    withContext(factoryContext.ioDispatcher) {
      ...
      val portLine = output.lines()
        .firstOrNull { it.trimStart().startsWith("Port ${args.ccEctoolPort}:") }
  ...
  /**
   * Polls CC orientation while USB tests are running.
   * Updates [TypeCCcPhase.DETECTING_FIRST.detected] with the first non-unknown orientation found.
   * Intentionally returns early once detected; the caller cancels this coroutine on test completion.
   */
  ...
  /**
   * Full lifecycle for one port: wait for device → (auto-format if needed) → speed test.
   * Runs entirely on [factoryContext.ioDispatcher]; starts the moment the device is inserted.
   */
  ...
  private suspend fun runHdmiTest(): Boolean {
    ...
    try {
      while (true) {
        ...
        if (isConnected) {
          // Manual mode
          val displayManager = factoryContext.applicationContext.getSystemService(Context.DISPLAY_SERVICE) as DisplayManager
            ...
            val monitorJob = launch {
              while (true) {
                ...
                if (statusTextMonitor != "connected") {
                  ...
                  _ui.update { it.copy(hdmiTestPhase = ExternalDisplayTestPhase.WAITING_FOR_CONNECTION) }
                  updatePortStatus(3, "Waiting for HDMI display...")
                  break
                }
              }
   ...
   *
   * When [autoFormat] is true and the device is non-FAT32, it is formatted automatically
   * (once per device per run — tracked by [formattedDevices]).
   */
  private suspend fun findBlockDevForPrefix(
    ...
    withContext(factoryContext.ioDispatcher) {
      try {
        val sdDevs = adb("ls /sys/block/ 2>/dev/null").outText.lines().filter { it.startsWith("sd") }
        ...
        for (dev in sdDevs) {
          ...
          for (targetDev in diskDevs.reversed()) {
            ...
            val fsType = Regex("""TYPE="([^"]+)"""").find(blkidOut)?.groupValues?.get(1) ?: continue

            // Accept vfat or exfat (both are fast, portable native formats).
            if (fsType.equals("vfat", ignoreCase = true) || fsType.equals("exfat", ignoreCase = true)) {
              val uuid = Regex("""UUID="([^"]+)"""").find(blkidOut)?.groupValues?.get(1) ?: continue
              val mountPoint = "/mnt/media_rw/$uuid"
              val mountOk = adb("test -d $mountPoint && echo OK").outText.trim() == "OK"

              if (mountOk) {
                return@withContext PortDeviceC(blockDev = targetDev, mountPoint = mountPoint, uuid = uuid)
              } else {
   ...
   * `/sys/block/`, regardless of filesystem type or mount state.
   * Used only to drive the UI connection dot — does NOT imply the device is ready for testing.
   */
  ...
  /**
   * Polls until a [PortDeviceC] is found for [prefix].
   * The overall test timeout handles cancellation.
   * Updates the UI connection dot on every cycle.
   */
  private suspend fun waitForPortDeviceC(prefix: String, label: String, portIndex: Int): PortDeviceC? {
    var attempts = 0
    while (true) {
      ...
      val dev = findBlockDevForPrefix(prefix, autoFormat = args.autoFormatToFat32, portIndex = portIndex)
      if (dev != null) {
        updatePort(portIndex) { it.copy(isPresent = true) }
        return dev
      }
      attempts++
      val statusMsg = if (present)
        "$label detected — preparing... ($attempts)"
      else
        "Waiting for $label stick... ($attempts)"
  ...
  /**
   * Formats [blockDev] using Android's built-in volume daemon (vold).
   * `sm partition disk:X,Y public` safely unmounts, partitions, and formats as FAT32/exFAT.
   */
  private suspend fun formatDeviceToFat32(prefix: String, blockDev: String, portIndex: Int): Boolean =
    withContext(factoryContext.ioDispatcher) {
      ...
      repeat(maxPolls) { i ->
        ...
        val targets = partsOut.lines().filter { it.startsWith(blockDev) && it != blockDev }.toMutableList()
  ...
  private suspend fun performSpeedTest(
    ...
    val testFile = File(mountPoint, "FactoryApp/SpeedTest_$portIndex.bin").apply {
      parentFile?.mkdirs()
      if (exists()) delete()
    }
    ...
    try {
      ...
      val writeOut = adb("dd if=/dev/zero of=${testFile.absolutePath} bs=${fileSizeMb}M count=1 conv=fsync")
      ...
      if (!passed) {
        Log.error(
          "[$label] FAILED — Write=%.1f(≥%.1f) Read=%.1f(≥%.1f) MB/s".format(
            writeSpeed, minWrite, readSpeed, minRead,
          )
        )
      }
  ...
  private fun parseDdSpeed(output: String): Double? {
    val match = """,\s*(\d+\.?\d*)\s*([KMG])(?:B?)/s""".toRegex().find(output) ?: run {
  ...
  private fun updatePortStatus(portIndex: Int, status: String, finished: Boolean = false, passed: Boolean = false) =
